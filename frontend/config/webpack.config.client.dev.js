...
const config = {
  ...
  entry: [
    ...
    paths.appIndexJs,
    // We include the app code last so that if there is a runtime error during initialization, it doesn't blow up the WebpackDevServer client, and changing JS code would still trigger a refresh.
  ],
  output: {
    ...
    rules: [
      ...
      },
      {
        oneOf: [
          ...
          // In production, we use a plugin to extract that CSS to a file, but in development "style" loader enables hot editing of CSS.
          {
            test: /\.css$/,
            use: [
  ...
  plugins: [
    ...
    // If you require a missing module and then `npm install` it, you still have to restart the development server for Webpack to discover it. This plugin
