...
module.exports = function(proxy, allowedHost) {
  return {
    ...
    // While we're investigating better solutions, for now we will take a compromise. Since our WDS configuration only serves files in the `public` folder we won't consider accessing them a vulnerability. However, if you use the `proxy` feature, it gets more dangerous because it can expose remote code execution vulnerabilities in backends like Django and Rails.
    // So we will disable the host check normally, but enable it if you have specified the `proxy` setting. Finally, we let you override it if you really know what you're doing with a special environment variable.
    disableHostCheck:
    ...
    // Enable hot reloading server. It will provide /sockjs-node/ endpoint for the WebpackDevServer client so it can learn when the files were updated. The WebpackDevServer client is included as an entry point in the Webpack development configuration. Note that only changes to CSS are currently hot reloaded. JS changes will refresh the browser.
    // hot: true,
    // It is important to tell WebpackDevServer to use the same "root" path
    // as we specified in the config. In development, we always serve from /.
    publicPath: config.output.publicPath,
    // WebpackDevServer is noisy by default so we emit custom message instead by listening to the compiler events with `compiler.plugin` calls above.
    quiet: true,
